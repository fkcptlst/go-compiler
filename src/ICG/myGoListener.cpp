...
string myGoListener::CreateLocalVar() {
  ...
  /* TODO: 判断tmp类型 */
  std::shared_ptr<Symbol> symbol = make_shared<Symbol>(
      Local, currentScope, Symbol::SymbolType::VAR, defineTmpType());
...
void myGoListener::Go23file(string filename) {
  ...
  for (auto p : TACBlocks) {
    ...
    for (auto it : *block) {
      ...
      outfile << left << setw(4) << it.line << setw(15) << ToString(it.op)
              << setw(10) << ToString(it.src1.OperType) + ":" << setw(10)
              << it.src1.value << setw(10) << ToString(it.src2.OperType) + ":"
              << setw(10) << it.src2.value << setw(10)
              << ToString(it.dst.OperType) + ":" << setw(10) << it.dst.value
              << endl;
    }
    outfile << "\n"; // end
  }
...
void myGoListener::Go23file_(string filename) {
  ...
  for (auto p : TACBlocks) {
    ...
    for (auto it : *block) {
      ...
      outfile << it.line << "\t" << ToString(it.op) << "\t" << it.src1.value
              << "\t" << it.src2.value << "\t" << it.dst.value << "\t" << endl;
    }
    outfile << "----------------------\n";
  }
  outfile.close();
...
// 第一个参数，改成函数名字
void myGoListener::push_line(TACOP op, Operand src1, Operand src2,
                             Operand dst) {
  TACBlocks[curFun]->push_back(
      TACLine(myGoListener::LineIndex, op, src1, src2, dst, currentScope));
...
void myGoListener::exitBasicLit(GoParser::BasicLitContext *ctx) {
  ...
  if (ctx->integer()) {
    ...
    // Operand(BasicLitValue,TACOPERANDTYPE::IMM),Operand("",TACOPERANDTYPE::NULL_),
    // Operand(dst,TACOPERANDTYPE::VAR)); values->put(ctx, dst);
...
void myGoListener::exitPrimaryExpr(GoParser::PrimaryExprContext *ctx) {
  ...
  if (ctx->arguments()) {
    ...
    /*找到了函数*/
    std::shared_ptr<vector<string>> arguments_values =
        ctx_decoder(values->get(ctx->arguments()));
    /*判断参数数量合理*/
    ...
    for (auto ret : *fun_symbol->fun_ret_type_list) {
      ...
      push_line(TACOP::RET, Operand(tmp, OperandTypereslove(tmp)),
                Operand(blank, TACOPERANDTYPE::NULL_),
                Operand(blank, TACOPERANDTYPE::NULL_));
      primaryExpr_value.push_back(tmp);
    }
    for (auto para : *arguments_values) {
      push_line(TACOP::PARA, Operand(para, OperandTypereslove(para)),
                Operand(blank, TACOPERANDTYPE::NULL_),
                Operand(blank, TACOPERANDTYPE::NULL_));
    }
    push_line(TACOP::CALL, Operand(identity, OperandTypereslove(identity)),
              Operand(blank, TACOPERANDTYPE::NULL_),
              Operand(blank, TACOPERANDTYPE::NULL_));
  ...
  /* 数组 */
    ...
    string int_size = "4";
    push_line(TACOP::MUL, Operand(int_size, OperandTypereslove(int_size)),
    ...
    push_line(TACOP::ADD, Operand(identity, OperandTypereslove(identity)),
              Operand(tmp_ptr_offset, OperandTypereslove(tmp_ptr_offset)),
...
void myGoListener::enterPrimaryExpression(
    GoParser::PrimaryExpressionContext *ctx) {}
void myGoListener::exitPrimaryExpression(
    GoParser::PrimaryExpressionContext *ctx) {
...
void myGoListener::enterPlusMinusOperation(
    GoParser::PlusMinusOperationContext *ctx) {}
void myGoListener::exitPlusMinusOperation(
    GoParser::PlusMinusOperationContext *ctx) {
  my_func("exitPlusMinusOperation");

  std::shared_ptr<vector<string>> left =
      ctx_decoder(values->get(ctx->expression(0)));
  std::shared_ptr<vector<string>> right =
      ctx_decoder(values->get(ctx->expression(1)));
  /*判断value是否只有一个*/
  ...
  if (ctx->PLUS()) {
    push_line(TACOP::ADD, Operand((*left)[0], OperandTypereslove((*left)[0])),
              Operand((*right)[0], OperandTypereslove((*right)[0])),
              Operand(dst, OperandTypereslove(dst)));
  } else if (ctx->MINUS()) {
    push_line(TACOP::SUB, Operand((*left)[0], OperandTypereslove((*left)[0])),
              Operand((*right)[0], OperandTypereslove((*right)[0])),
              Operand(dst, OperandTypereslove(dst)));
  }
...
void myGoListener::enterRelationOperation(
    GoParser::RelationOperationContext *ctx) {}
void myGoListener::exitRelationOperation(
    GoParser::RelationOperationContext *ctx) {
  ...
  // expression 在for 下的情况，需要添加ifexp 并且反过来
  // 将condition_loop信息写入forstatement里面
  if (ctx->parent->parent->children[0]->getText() == "for" &&
      ctx->parent->parent->children[1] == ctx->parent) {
    std::string dst, dst_;
    ...
    TACLine tmpline;
    if (ctx->EQUALS()) {
      tmpline = TACLine(myGoListener::LineIndex, TACOP::IFEQ,
                        Operand((*left)[0], OperandTypereslove((*left)[0])),
                        Operand((*right)[0], OperandTypereslove((*right)[0])),
                        Operand(dst_, TACOPERANDTYPE::LABEL), currentScope);

      push_line(TACOP::IFNEQ,
                Operand((*left)[0], OperandTypereslove((*left)[0])),
                Operand((*right)[0], OperandTypereslove((*right)[0])),
                Operand(dst, TACOPERANDTYPE::LABEL));
    } else if (ctx->NOT_EQUALS()) {
      tmpline = TACLine(myGoListener::LineIndex, TACOP::IFNEQ,
                        Operand((*left)[0], OperandTypereslove((*left)[0])),
                        Operand((*right)[0], OperandTypereslove((*right)[0])),
                        Operand(dst_, TACOPERANDTYPE::LABEL), currentScope);
      ...
      tmpline = TACLine(myGoListener::LineIndex, TACOP::IFGT,
                        Operand((*left)[0], OperandTypereslove((*left)[0])),
                        Operand((*right)[0], OperandTypereslove((*right)[0])),
                        Operand(dst_, TACOPERANDTYPE::LABEL), currentScope);
      ...
      tmpline = TACLine(myGoListener::LineIndex, TACOP::IFGE,
                        Operand((*left)[0], OperandTypereslove((*left)[0])),
                        Operand((*right)[0], OperandTypereslove((*right)[0])),
                        Operand(dst_, TACOPERANDTYPE::LABEL), currentScope);
      ...
      tmpline = TACLine(myGoListener::LineIndex, TACOP::IFLT,
                        Operand((*left)[0], OperandTypereslove((*left)[0])),
                        Operand((*right)[0], OperandTypereslove((*right)[0])),
                        Operand(dst_, TACOPERANDTYPE::LABEL), currentScope);
      ...
      tmpline = TACLine(myGoListener::LineIndex, TACOP::IFLE,
                        Operand((*left)[0], OperandTypereslove((*left)[0])),
                        Operand((*right)[0], OperandTypereslove((*right)[0])),
                        Operand(dst_, TACOPERANDTYPE::LABEL), currentScope);
  ...
  if (ctx->parent->getText().find("if") == 0 &&
    ...
    // push_line (TACOP::IFEXP, Operand(dst, OperandTypereslove(dst)),
...
void myGoListener::exitMulDivOperation(GoParser::MulDivOperationContext *ctx) {
  ...
  if (ctx->STAR()) {
    push_line(TACOP::MUL, Operand((*left)[0], OperandTypereslove((*left)[0])),
              Operand((*right)[0], OperandTypereslove((*right)[0])),
              Operand(dst, OperandTypereslove(dst)));
  } else if (ctx->DIV()) {
    push_line(TACOP::DIV, Operand((*left)[0], OperandTypereslove((*left)[0])),
              Operand((*right)[0], OperandTypereslove((*right)[0])),
              Operand(dst, OperandTypereslove(dst)));
  }
...
void myGoListener::enterLogicalOrOperation(
    GoParser::LogicalOrOperationContext *ctx) {}
void myGoListener::exitLogicalOrOperation(
    GoParser::LogicalOrOperationContext *ctx) {}

void myGoListener::enterLogicalAndOperation(
    GoParser::LogicalAndOperationContext *ctx) {}
void myGoListener::exitLogicalAndOperation(
    GoParser::LogicalAndOperationContext *ctx) {}
...
void myGoListener::enterFunctionDecl(GoParser::FunctionDeclContext *ctx) {
  ...
  /* 构建fun symbol */
  std::shared_ptr<vector<Symbol::Type>> funRetTypeList =
      make_shared<vector<Symbol::Type>>();
  std::shared_ptr<vector<Symbol::Type>> funParaList =
      make_shared<vector<Symbol::Type>>();
  /*函数ret参数*/
  if (!ctx->signature()->result() ||
      ctx->signature()->result()->parameters()->parameterDecl().size() ==
          0) { // 无返回值
               // do nothing
  } else {     // 有返回值
    int n = ctx->signature()->result()->parameters()->parameterDecl().size();
    for (int i = 0; i < n; ++i) {
      string eachType = ctx->signature()
                            ->result()
                            ->parameters()
                            ->parameterDecl(i)
                            ->type_()
  ...
  std::shared_ptr<Symbol> symbol =
      make_shared<Symbol>(identifier, currentScope, Symbol::SymbolType::FUN,
                          funRetTypeList, funParaList);
  std::shared_ptr<Scope> scope = make_shared<Scope>(currentScope);
  myGoListener::currentScope->fun_define(symbol);
  currentScope = scope;

  /*打印 FUN_PARA*/
  for (int i = 0; i < ctx->signature()->parameters()->parameterDecl().size();
       ++i) {
    // int
    // n=ctx->signature()->parameters()->parameterDecl()->identifierList()->IDENTIFIER().size();
    int para_number = ctx->signature()
                          ->parameters()
                          ->parameterDecl(i)
                          ->identifierList()
                          ->IDENTIFIER()
    ...
    for (int j = 0; j < para_number; ++j) {
      std::string fun_para = ctx->signature()
                                 ->parameters()
                                 ->parameterDecl(i)
                                 ->identifierList()
                                 ->IDENTIFIER(j)
      ...
      push_line(TACOP::FUN_PARA,
                Operand(fun_para, OperandTypereslove(fun_para)),
...
void myGoListener::exitVarSpec(GoParser::VarSpecContext *ctx) {
  ...
  if (ctx->type_() && ctx->type_()->typeLit()) {
    ...
    right_values = ctx_decoder(values->get(
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (!ctx->type_()) {
      ...
      /* 数组 */
      else if (ctx->type_()->typeLit() &&
               ctx->type_()->typeLit()->arrayType()) {
        string stype =
    ...
    /*不重复，新建*/
    std::shared_ptr<Symbol> symbol =
        make_shared<Symbol>(varname, currentScope, Symbol::SymbolType::VAR,
                            type, is_array, array_length);
    if (is_array) {
      push_line(TACOP::CREATLIST, Operand(varname, OperandTypereslove(varname)),
                Operand(std::to_string(array_length),
                        OperandTypereslove(std::to_string(array_length))),
  ...
  if (ctx->expressionList()) {
    ...
    for (int i = 0; i < n; ++i) {
      push_line(
          TACOP::ASSIGN,
          Operand((*right_values)[i], OperandTypereslove((*right_values)[i])),
          Operand("", TACOPERANDTYPE::NULL_),
...
void myGoListener::enterBlock(GoParser::BlockContext *ctx) {
  ...
  if (ctx->parent->children[0]->getText() == "for") {
    push_line(TACOP::LABEL,
              Operand("FORLOOP" + forvalues->get(ctx->parent).CurIndex,
                      TACOPERANDTYPE::LABEL),
              Operand("", TACOPERANDTYPE::NULL_),
              Operand("", TACOPERANDTYPE::NULL_));
  }
  // if else 情况
  if (ctx->parent->children[4] == ctx &&
      ctx->parent->children[3]->getText() == "else") {
    push_line(
        TACOP::LABEL,
        Operand("ELSE" + ifvalues->get(ctx->parent), TACOPERANDTYPE::LABEL),
        Operand("", TACOPERANDTYPE::NULL_), Operand("", TACOPERANDTYPE::NULL_));
  }
  addScope();
}
void myGoListener::exitBlock(GoParser::BlockContext *ctx) {
  ...
  if (ctx->parent->children[0]->getText() == "for") {
    ...
    push_line(updateCondition.op, updateCondition.src1, updateCondition.src2,
              updateCondition.dst);
    push_line(loopCondition.op, loopCondition.src1, loopCondition.src2,
              loopCondition.dst);
  }

  // if 情况
  if (ctx->parent->children[2] == ctx &&
      ctx->parent->children[0]->getText() == "if") {
    push_line(
        TACOP::GOTO,
        Operand("ENDIF" + ifvalues->get(ctx->parent), TACOPERANDTYPE::LABEL),
        Operand("", TACOPERANDTYPE::NULL_), Operand("", TACOPERANDTYPE::NULL_));
  }
  popScope();
}
...
void myGoListener::exitIncDecStmt(GoParser::IncDecStmtContext *ctx) {
  ...
  if (ctx->children[1]->getText() == "++") {
    ...
    // for statement
    // 的情况，不写入3code，但是要记录在forstmt里面，作为update的条件
    if (ctx->parent->parent->parent->children[0]->getText() == "for" &&
        ctx->parent->parent == ctx->parent->parent->parent->children[1]) {
      string varname = (*left_values)[0];
      string varvalue = "1";
      ForStmt fortmp = forvalues->get(ctx->parent->parent->parent);
      TACLine tmpline =
          TACLine(myGoListener::LineIndex, TACOP::ADD,
                  Operand(varname, OperandTypereslove(varname)),
                  Operand(varvalue, OperandTypereslove(varvalue)),
                  Operand(varname, OperandTypereslove(varname)), currentScope);
    ...
    // 普通情况，block里面，写入3code
      ...
      string varname = (*left_values)[0];
      string varvalue = "1";
      push_line(TACOP::ADD, Operand(varname, OperandTypereslove(varname)),
                Operand(varvalue, OperandTypereslove(varvalue)),
                Operand(varname, OperandTypereslove(varname)));
    }
  }
...
void myGoListener::exitAssignment(GoParser::AssignmentContext *ctx) {
  ...
  if (ctx->assign_op()->getText() == "=") {
    ...
    for (int i = 0; i < left_values->size(); i++) {
      ...
      push_line(TACOP::ASSIGN, Operand(varvalue, OperandTypereslove(varvalue)),
                Operand("", TACOPERANDTYPE::NULL_),
                Operand(varname, OperandTypereslove(varname)));
      // }
    }
...
void myGoListener::exitShortVarDecl(GoParser::ShortVarDeclContext *ctx) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    /*不重复，新建*/
    std::shared_ptr<Symbol> symbol = make_shared<Symbol>(
        varname, currentScope, Symbol::SymbolType::VAR, type);
    myGoListener::currentScope->para_define(symbol);
  }
  ...
  if (ctx->parent->parent->parent->children[0]->getText() == "for" &&
      ctx->parent->parent->parent->children[1] == ctx->parent->parent) {
...
void myGoListener::exitReturnStmt(GoParser::ReturnStmtContext *ctx) {
  ...
  /*生成TAC*/
  for (auto i : (*return_values)) {
    push_line(TACOP::FUN_RET, Operand(i, OperandTypereslove(i)),
              Operand("", TACOPERANDTYPE::NULL_),
              Operand("", TACOPERANDTYPE::NULL_));
  }
}
...
void myGoListener::exitIfStmt(GoParser::IfStmtContext *ctx) {
  ...
  push_line(TACOP::LABEL,
            Operand("ENDIF" + ifvalues->get(ctx), TACOPERANDTYPE::LABEL),
            Operand("", TACOPERANDTYPE::NULL_),
            Operand("", TACOPERANDTYPE::NULL_));
}
...
void myGoListener::exitForStmt(GoParser::ForStmtContext *ctx) {
  ...
  push_line(
      TACOP::LABEL,
      Operand("ENDFOR" + forvalues->get(ctx).CurIndex, TACOPERANDTYPE::LABEL),
      Operand("", TACOPERANDTYPE::NULL_), Operand("", TACOPERANDTYPE::NULL_));
}
...
void myGoListener::enterParameterDecl(GoParser::ParameterDeclContext *ctx) {
  ...
  if (ctx->identifierList()) { /* x int */
    ...
    for (int i = 0; i < n; ++i) {
      ...
      std::shared_ptr<Symbol> symbol = make_shared<Symbol>(
          integer, currentScope, Symbol::SymbolType::VAR, type);
      myGoListener::currentScope->para_define(symbol);
    }
  }
}
